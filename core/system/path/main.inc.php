...
if (!class_exists('csl_path')) {
	...
	class csl_path {
		 ...
		 * @return - string|boolean
		 * @usage - csl_path::nexus_full_relative($path,$scriptName);
		 */
		public static function nexus_full_relative($path = null, $scriptName = null) {
			if (!csl_func_arg :: delimit2error() && !csl_func_arg :: string2error(0) && !csl_func_arg :: string2error(1)) {
				...
				elseif (self :: is_absolute($scriptName) || (!self :: is_root_model($scriptName) && !self :: is_relative($scriptName))) {
					csl_error :: cast(__CLASS__ . '::' . __FUNCTION__ . '(): Invalid argument by parameter 2', E_USER_WARNING, 1);
				} else {
			...
			if (!csl_func_arg :: delimit2error() && !csl_func_arg :: string2error(0)) {
				if (self :: is_absolute($path) || (!self :: is_root_model($path) && !self :: is_relative($path))) {
					...
					if ($scriptDir !== false) {
						...
						csl_error :: cast(__CLASS__ . '::' . __FUNCTION__ . '(): Parent directories does not have the readable', E_USER_WARNING, 1);
					}
				}
			}
			return false;
		 ...
		 * @return - string|boolean
		 * @usage - csl_path::arrive($path,$mode);
		 */
		 ...
		 * @param - boolean $mode (keep empty string directory) : Default false
		 * @return - string|boolean
		 * @usage - csl_path::clean($path,$mode);
		 */
		...
		public static function document_root() {
			if (!csl_func_arg :: delimit2error() && isset ($_SERVER['DOCUMENT_ROOT']) && is_string($_SERVER['DOCUMENT_ROOT']) ) {
